# -*- coding: utf-8 -*-
"""
Created on Fri Dec 29 09:17:16 2017
...
target_url = "file:/D:/MyPaper/KDD99/corrected"
...
for k in range(0,750):
    for row in xList[500*k:500*(k+1)]:
        ...
        if row[2]=='X11': row[2]=68
        if row[2]=='Z39_50': row[2]=69
        if row[2]=='eco_i': row[2]=70
        if row[2]=='icmp': row[2]=71
        ## eco-i, icmp do not exist in train
        if row[3]=='OTH': row[3]=0
        if row[3]=='REJ': row[3]=1 
        if row[3]=='RSTO': row[3]=2 
        if row[3]=='RSTOS0': row[3]=3 
...
for l in labels:
    if l == 'back.': index=0
    if l == 'buffer_overflow.': index=1
    if l == 'ftp_write.': index=2
    if l == 'guess_passwd.': index=3
    if l == 'imap.': index=4
    if l == 'ipsweep.': index=5
    if l == 'land.': index=6
    if l == 'loadmodule.': index=7
    if l == 'multihop.': index=8
    if l == 'neptune.': index=9
    if l == 'nmap.': index=10
    if l == 'normal.': index=11
    if l == 'perl.': index=12
    if l == 'phf.': index=13
    if l == 'pod.': index=14
    if l == 'portsweep.': index=15
    if l == 'rootkit.': index=16
    if l == 'satan.': index=17
    if l == 'smurf.': index=18
    if l == 'spy..': index=19
    if l == 'teardrop.': index=20
    if l == 'warezclient.': index=21
    if l == 'warezmaster.': index=22
    if l == 'apache2.': index=23
    if l == 'httptunnel.': index=24
    if l == 'mailbomb.': index=25
    if l == 'mscan.': index=26
    if l == 'named.': index=27
    if l == 'processtable.': index=28
    if l == 'ps.': index=29
    if l == 'saint.': index=30
    if l == 'sendmail.': index=31
    if l == 'snmpgetattack.': index=32
    if l == 'snmpguess.': index=33
    if l == 'sqlattack.': index=34
    if l == 'udpstorm.': index=35
    if l == 'worm.': index=36
    if l == 'xlock.': index=37
    if l == 'xsnoop.': index=38
    if l == 'xterm.': index=39
    newLabels.append(index)
labelList=newLabels
...
#xTrain, xTest, yTrain, yTest = train_test_split(xTemp, yTemp, test_size=0.30, random_state=531)
xTemp0 = [xNum[i] for i in range(nrows) if newLabels[i] == 0]
xTemp = [xTemp0[i] for i in range(len(xTemp0))]
yTemp0 = [newLabels[i] for i in range(nrows) if newLabels[i] == 0]
yTemp = [yTemp0[i] for i in range(len(yTemp0))]
#print xTemp
xTest2=xTemp0
yTest2=yTemp0
#xTrain, xTest, yTrain, yTest = train_test_split(xTemp, yTemp, test_size=0.30, random_state=531)
#print len(xTrain), len(yTest)
for iLabel in range(1, 40):
    #segregate x and y according to labels
    xTemp = [xNum[i] for i in range(nrows) if newLabels[i] == iLabel]
    yTemp = [newLabels[i] for i in range(nrows) if newLabels[i] == iLabel]
    print iLabel
    ...
    #yTrain = numpy.append(yTrain, yTrainTemp, axis=0); yTest = numpy.append(yTest, yTestTemp, axis=0)
    if xTemp!=[]:
        xTest2 = numpy.append(xTest2, xTemp, axis=0); yTest2 = numpy.append(yTest2, yTemp, axis=0)
missCLassError = []
missCLassError2 = []
list=[]
mlp = MLPClassifier(solver='adam', alpha=0.01,hidden_layer_sizes=(5, 5), random_state=1, activation='relu', learning_rate_init=0.01,max_iter=1000)
mlp.fit(xTrain,yTrain)
    #Accumulate auc on test set
#   prediction = attackRFModel.predict(xTest)
prediction = mlp.predict(xTest)
prediction2 = mlp.predict(xTest2)
correct = accuracy_score(yTest, prediction)
correct2 = accuracy_score(yTest2, prediction2)
missCLassError.append(1.0 - correct)
missCLassError2.append(1.0 - correct2)
print("Missclassification Error" )
print(missCLassError[-1])
print(missCLassError2[-1])
