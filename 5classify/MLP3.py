...
for l in labels:
    if l == 'back.': index=2
    if l == 'buffer_overflow.': index=3
    if l == 'ftp_write.': index=4
    if l == 'guess_passwd.': index=4
    if l == 'imap.': index=4
    if l == 'ipsweep.': index=1
    if l == 'land.': index=2
    if l == 'loadmodule.': index=3
    if l == 'multihop.': index=4
    if l == 'neptune.': index=2
    if l == 'nmap.': index=1
    if l == 'normal.': index=0
    if l == 'perl.': index=3
    if l == 'phf.': index=4
    if l == 'pod.': index=2
    if l == 'portsweep.': index=1
    if l == 'rootkit.': index=3
    if l == 'satan.': index=1
    if l == 'smurf.': index=2
    if l == 'spy.': index=4
    if l == 'teardrop.': index=2
    if l == 'warezclient.': index=4
    if l == 'warezmaster.': index=4
    if l == 'apache2.': index=2
    if l == 'httptunnel.': index=3
    if l == 'mailbomb.': index=2
    if l == 'mscan.': index=1
    if l == 'named.': index=4
    if l == 'processtable.': index=2
    if l == 'ps.': index=3
    if l == 'saint.': index=1
    if l == 'sendmail.': index=4
    if l == 'snmpgetattack.': index=4
    if l == 'snmpguess.': index=4
    if l == 'sqlattack.': index=3
    if l == 'udpstorm.': index=2
    if l == 'worm.': index=4
    if l == 'xlock.': index=4
    if l == 'xsnoop.': index=4
    if l == 'xterm.': index=3
